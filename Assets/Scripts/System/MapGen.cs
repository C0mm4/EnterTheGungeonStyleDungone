...
public class MapGen
{
    ...
    public List<Vector2> AblePos { get
        {
            return ablepos.Keys.ToList();
        } 
    }
    ...
    // For Test
    public void AddRoomForTest()
    {
        var room = DungeonRoomGen();
        Rooms.Add(room);
    }
    ...
    public DunRoom DungeonRoomGen()
    {
        ...
        do
        {
            ...
            po = Utility.GetPointPosition(targetPos, pos);
            switch (po)
            {
            ...
            for(int j = room.y - room.centerY - 4; j <  room.y + room.sizeY - room.centerY + 4; j++)
            {
                ...
                if ((i == room.x - room.centerX - 4 || i == room.x + room.sizeX - room.centerX + 3) || (j == room.y - room.centerY - 4 || j == room.y + room.sizeY - room.centerY + 3))
                {
                    ...
                    else
                    {
                        ...
                        {
                            PointPosition po = Utility.GetPointPosition(new Vector2(room.x, room.y), pos);
